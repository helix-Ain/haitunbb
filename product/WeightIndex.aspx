<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WeightIndex.aspx.cs" Inherits="Digicloud.DAM.Web.CI.Mobile.WeightIndex" %>
	...
	<body ng-controller='app' ng-init='init()' ng-cloak>
		...
		<ion-content class='has-tabs-top' style='background-color: #E2EEF6;'>
			<ion-list>
				...
				<ion-item style='text-indent: 2em;border:none;padding:10px 15px 10px;'>
					<span style='white-space: normal;font-size:15px;'>宝宝{{age}}了，宝宝的生理指标正常均值为体重：{{fMinWeight}}~{{fMaxWeight}}kg。</span>
				</ion-item>
		...
		<script>
			app.controller('app', function($scope, $window, commonService, initService) {
						...
						success: function(ci_result) {
							if(existData(ci_result)) {
								...
								angular.forEach(m_objData, function(value, key) {
									...
									data.push(value.fWeight);
									fMinWeight.push(value.fMinWeight);
									fMaxWeight.push(value.fMaxWeight);
								});
								loadChart(labels, data, fMinWeight, fMaxWeight);
							} else {
								$scope.tip = true;
							}
						}
					...
					commonService.postData({
						...
						success: function(ci_result) {
							if(existData(ci_result)) {
								...
								$scope.fMinWeight = m_objData.fCMinWeight.toFixed(2);
								$scope.fMaxWeight = m_objData.fCMaxWeight.toFixed(2);
							} else {
				...
				function loadChart(labels, data, fMinWeight, fMaxWeight) {
